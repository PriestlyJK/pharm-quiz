...
// ── SHUFFLE OPTIONS (fix for answer bias) ────────────────────────────
function shuffleOpts(q) {
  const indexed = q.o.map((text, i) => ({text, isCorrect: i === q.a}));
  for (let i = indexed.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [indexed[i], indexed[j]] = [indexed[j], indexed[i]];
  }
  const newCorrect = indexed.findIndex(x => x.isCorrect);
  return {opts: indexed.map(x => x.text), correct: newCorrect};
}

function renderMCQ(q, top) {
  const {opts: sOpts, correct: sCorrect} = shuffleOpts(q);
  const optsHtml = sOpts.map((o, i) => `
    <button class="opt" onclick="checkMCQ(${i}, ${sCorrect})">
